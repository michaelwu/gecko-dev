                        ...
                        gfxImageFormat aFormat, uint8_t aPaletteDepth /* = 0 */)
{
  ...
  if (aPaletteDepth != 0) {
    ...
    if (!mImageSurface) {
      ...
      mImageSurface = new gfxLockedImageSurface(mVBuf, mSize, mFormat);
    }
...
nsresult imgFrame::Optimize()
{
  ...
  if (optFormat == gfxImageFormat::RGB16_565) {
    RefPtr<VolatileBuffer> buf =
      gfxLockedImageSurface::AllocateBuffer(mSize, optFormat);
    if (!buf)
      return NS_OK;

    nsRefPtr<gfxLockedImageSurface> surf =
      new gfxLockedImageSurface(buf, mSize, optFormat);
...
nsresult imgFrame::LockImageData()
{
  ...
  if (!mImageSurface) {
    ...
    if (mOptSurface || mSinglePixel || mFormat == gfxImageFormat::RGB16_565) {
      ...
      // Recover the pixels
      RefPtr<VolatileBuffer> buf =
        gfxLockedImageSurface::AllocateBuffer(mSize, format);
      if (!buf) {
        return NS_ERROR_OUT_OF_MEMORY;
      }

      RefPtr<gfxLockedImageSurface> surf =
        new gfxLockedImageSurface(buf, mSize, format);
